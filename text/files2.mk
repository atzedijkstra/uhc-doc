...
RULER_12_DRV_LTEX			:= $(patsubst $(SRC_EHC_RULES_PREFIX)%.rul,$(TEXT_TMP_VARIANT_PREFIX)%.ltex,$(EHC_RULES_1_SRC_RUL) $(EHC_RULES_2_SRC_RUL))
RULER_12_DRV_TEX			:= $(RULER_12_DRV_LTEX:.ltex=.tex)

RULER_3_DRV_LTEX			:= $(patsubst $(SRC_EHC_RULES_PREFIX)%.rul,$(TEXT_TMP_VARIANT_PREFIX)%.ltex,$(EHC_RULES_3_SRC_RL2))
...
TEXT_SUBS_DRV_TEX			:= $(if $(TEXT_CFG_TEXT_INCLUDES_PREV_RULER_TEX),$(RULER_12_DRV_TEX)) \
								$(if $(TEXT_CFG_TEXT_INCLUDES_RULER_TEX),$(RULER2_RULES_DRV_TEX) $(RULER_3_DRV_TEX) $(RULER_4_DRV_TEX)) \
								$(if $(TEXT_CFG_TEXT_INCLUDES_RULER_TEX),$(TEXT_RULES_3_DRV_TEX) $(TEXT_RULES_MISC_DRV_TEX))
...
TEXT_ALL_PDFONLY_DPD		:= $(TEXT_MAIN_DRV_TEX) $(TEXT_SUBS_DRV_TEX) $(TEXT_MAIN_DRV_STY) \
								...
								$(if $(TEXT_CFG_TEXT_INCLUDES_RULER_TEX),$(TEXT_GEN_BY_RULER_TABLE_TEX) $(TEXT_RULEUX_ALL_DRV_TEX) $(TEXT_RULEUX_ALL_DRV_TEX) $(TEXT_RULEX_ALL_DRV_TEX)) \
								$(if $(TEXT_CFG_TEXT_INCLUDES_INF2PS_TEX),$(TEXT_INF2PS_ALL_DRV_TEX)) \
...
TEXT_SUBS_SHUFFLE1			:= $(TEXT_SUBS_SRC_CLTEX) \
								$(if $(TEXT_CFG_TEXT_INCLUDES_RULER_TEX),$(TEXT_RULES_3_DRV_CAG) $(RULER2_ALL_CHUNK_SRC) $(AGPRIMER_ALL_CHUNK_SRC) $(TEXT_RULES_EXPLAIN_3_DRV_CAG)) \
								$(if $(TEXT_CFG_TEXT_INCLUDES_RULER_TEX),$(TEXT_INF2PS_SRC_CRUL)) \
...
$(RULER_12_DRV_LTEX) : $(TEXT_TMP_VARIANT_PREFIX)%.ltex : $(SRC_EHC_RULES_PREFIX)%.rul $(RULER1)
	mkdir -p $(@D)
	$(RULER1) --latex --base=$(*F) $< > $@

$(RULER_3_DRV_LTEX) : $(TEXT_TMP_VARIANT_PREFIX)%.ltex : $(SRC_EHC_RULES_PREFIX)%.rul $(RULER2)
	mkdir -p $(@D)
...
$(TEXT_INCL_LIST_TEX): $(TEXT_ALL_MK_FILES)
	@(for f in $(sort $(notdir $(TEXT_SUBS_DRV_TEX) \
	                           ...
	                           $(if $(TEXT_CFG_TEXT_INCLUDES_INF2PS_TEX),$(TEXT_INF2PS_ALL_DRV_TEX) ) \
	                           $(if $(TEXT_CFG_TEXT_INCLUDES_RULER_TEX),$(RULER_4_DRV_TEX) $(RULER_3_DRV_TEX) $(TEXT_RULEUX_ALL_DRV_TEX) $(TEXT_RULEX_ALL_DRV_TEX)) \
	            )      ) ; \
	  do \
	    echo "\\input" $$f ; \
	  done \
